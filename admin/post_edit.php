        ...
        fetch(`ajax_get_revisions.php?post_id=${postId}`).then(r => r.json()).then(data => {
            ...
            if (data.success && data.revisions) {
                if (data.revisions.length === 0) {
                    list.innerHTML = '<div style="padding:40px; text-align:center;">Nincs korábbi verzió.</div>';
                } else {
                    let html = '<table style="width:100%; border-collapse:collapse;">';
                    data.revisions.forEach(rev => {
                        html += `<tr style="border-bottom:1px solid #f1f5f9;">
                            <td style="padding:15px;"><strong>${rev.created_at}</strong><br><small>${rev.admin_name || 'Ismeretlen'}</small></td>
                ...
                list.innerHTML = '<div style="padding:40px; text-align:center; color:red;">Hálózati / szerver hiba.</div>';
            }
        }).catch(err => {
            document.getElementById('revisions-list').innerHTML = '<div style="padding:40px; text-align:center; color:red;">Hálózati hiba.</div>';
        });
    }
    ...
    function restoreRevision(id) {
        ...
        fetch(`ajax_get_revisions.php?revision_id=${id}`).then(r => r.json()).then(data => {
            if (data.success && data.revision) {
                document.querySelector('input[name="title"]').value = data.revision.title;
                if (tinymceInstance) tinymceInstance.setContent(data.revision.content);
                closeRevisionsModal();
                alert('Verzió visszatöltve! (Ne felejtsd el elmenteni)');
            } else {
                alert('Hiba: ' + (data.error || 'Szerver hiba.'));
            }
        });
    }
